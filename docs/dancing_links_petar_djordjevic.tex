...
Da bi mogao algoritam u potpunosti da se opiše, potrebno je dodati još dve operacije:


\[
\texttt{cover(x)} =
\begin{cases} 
\text{For } (p \gets D[x]; \quad p \neq x; \quad p \gets D[p])\\
\quad \quad \texttt{hide(p)}\\
\texttt{link(x)}
\end{cases}
\]
...
\begin{cases} 
\texttt{unlinkh(x)} \\
\text{For } (p \gets U[x]; \quad p \neq x; \quad p \gets U[p]) \\
\quad \quad \texttt{unhide(p)}
\end{cases}
\]


Mogu se formulisati i dodatne pomoćne funkcije za pokrivanje celog reda i za biranje minimalne kolone (kolona koja ima najmanji broj
  čvorova unutar sebe), ali zbog različitih potencijalnih implementacija ovde neće biti opisane.

\subsection{Algoritam igrajućih linkova}

Algoritam pretrage strukture igrajućih linkova može se predstaviti rekurzivno (usled sličnosti sa backtrackingom).
Kada pokrijemo kolonu, uklanjamo sve redove koji su povezani sa tom kolonom jer ih nećemo koristiti u trenutnom rešenju.
Kada se vratimo iz rekurzije, moramo vratiti te redove nazad kako bismo mogli da razmotrimo druge mogućnosti.
Izbor kolone sa najmanje čvorova omogućava brže sužavanje pretrage i smanjenje mogućnosti pogrešnih grana u pretrazi.

U pseudokodu, algoritam se može opisati na naredni način:

\begin{algorithm}
  \caption{Pretraži (Dancing Links Algorithm)}
  \begin{algorithmic}[1]
  \Function{Pretraži}{Rešenja, TrenutnoRešenje}
      \If {SveKolonePokrivene()}
          \State DodajURešenja(TrenutnoRešenje)
          \State \Return
      \EndIf
      \State Kolona $\gets$ IzaberiKolonuSaNajmanjeČvorova()
      \State PokrijKolonu(Kolona)
      \For{Čvor \textbf{in} Kolona}
          \State DodajRedUTrenutnoRešenje(Čvor)
          \For{DesniČvor \textbf{in} Čvor.Red}
              \State PokrijKolonu(DesniČvor.Kolona)
          \EndFor
          \State \Call{Pretraži}{Rešenja, TrenutnoRešenje}
          \State UkloniRedIzTrenutnogRešenja(Čvor)
          \For{LeviČvor \textbf{in} Čvor.Red}
              \State OtkrijKolonu(LeviČvor.Kolona)
          \EndFor
      \EndFor
      \State OtkrijKolonu(Kolona)
  \EndFunction
  \end{algorithmic}
  \end{algorithm}


Iako biranje kolone sa najmanjim brojem čvorova može delovati kao najefikasnija strategija za pokrivanje u algoritmu igrajućih
  linkova, često je korisno uvesti element pseudonasumičnosti u izbor kolone. Najčešća metoda je da se, u slučaju kada više kolona
  ima isti minimalni broj čvorova, izabere jedna kolona nasumično. Alternativno, može se implementirati potpuna pseudonasumičnost,
  gde se kolone biraju prema unapred definisanoj nasumičnoj strategiji.Uvođenjem nasumičnog rasporeda kolona ili redova u početku
  algoritma može se poboljšati efikasnost pretrage i smanjiti mogućnost zaglavljivanja u lokalnim minimumima.
...
\begin{enumerate}
  \item U svako polje se može upisati samo jedan od 9 brojeva. To treba predstaviti kao ograničenje, što znači da moramo uvesti
    81 uslov za svako polje, tako da se ne mogu 2 broja upisati u jedno polje.
  \item Svaki broj se može pojaviti samo jednom u svakom redu. To predstavlja novih 9 uslova, gde red koji predstavlja isti broj
    u istom redu dobija 1 u tablici. To je dodatnih 81 uslova.
  \item Svaki broj se može pojaviti samo jednom u svakoj koloni Poput reda, uvodi se 81 uslova za ovo.
  \item Svaki broj se može pojaviti samo jednom u svakoj podmreži 3x3. Poput prethodnih primera dobija se 81 uslov.
\end{enumerate}
Ukupno, tablica ograničenja je dimenzije 729x324. 
